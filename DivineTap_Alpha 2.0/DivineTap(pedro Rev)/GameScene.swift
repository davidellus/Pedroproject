...
//            addChild(spell)
            ...
            duration: TimeInterval(CGFloat(4.0)))
            let audioAction = SKAction.playSoundFileNamed("Colpo_Strega", waitForCompletion: false)
...
class Bat: Nodo{
    ...
    init(){
            ...
            life = 5
             ...
             self.physicsBody = SKPhysicsBody(circleOfRadius: 170)// 1
             self.physicsBody?.isDynamic = true // 2
...
//QUESTI SONO I VARI LAYER: posZ ci indica la posizione del layer (più il numero è negativo, più il layer starà dietro agli altri); se due layer hanno lo stesso posZ, allora quello che dichiariamo prima starà sotto quello dichiarato dopo
    ...
    playerHealthBar.position = CGPoint(x: 50, y: 50)
...
//DEFINIZIONE DELLE VARIE FUNZIONI
    func controllaScore(){
        checkround += 1
        var calma = checkround/2
        if score >= 100 && flagBoss == 0{
            flagBoss = 1
            addBoss()
            self.removeAction(forKey: "spawnSkeletons")
            self.removeAction(forKey: "spawnBats")
//            animate(spriteNode: boss, arrayFrames: boss.idleFrames)
        }
        else if score < 100{
            print("round \(checkround) \n")
                addBat()
                addBat()
                addSkeleton()
                if(checkround > 3){
                    for i in 0 ..< checkround-calma{
                        if checkround-calma <= 2+calma{
                        addSkeleton()
                        addBat()
                        }
                        else {
...
//        skeletons spawnano di dimensioni diverse
            ...
            skeleton.zPosition = 10
            skeleton.setScale(dimension)
            addChild(skeleton)
        let actualDuration = random(min: CGFloat(2.0), max: CGFloat(2.6))
            ...
            let actionMove = SKAction.move(to: CGPoint(x: -skeleton.size.width/2, y: size.height*0.3),
                                           duration: TimeInterval(CGFloat(actualDuration)))
            let actionMoveDone = SKAction.removeFromParent()
...
//    FUNZIONE PER L'AGGIUNTA DEI PIPISTRELLI (NECESSITA DELLE DUE FUNZIONI RANDOM PRECEDENTI
    func addBat() {
      ...
      // Determine speed of the bat
        let actualDuration = random(min: CGFloat(1.2), max: CGFloat(1.7))
    ...
    func addBoss(){
        let sound = Bundle.main.path(forResource: "soundBoss", ofType: "mp3")
...
//        ripetiamo all'infinito la sequenza composta da due parti: la prima parte della sequenza avvia la funzione add bat, mentre la seconda parte della sequenza da una pausa di 1 secondo: in questo modo otteiamo che viene generato un mob ogni secondo
        run(SKAction.repeatForever(
          SKAction.sequence([
            SKAction.run(addBat),
  ...
  func random(min: CGFloat, max: CGFloat) -> CGFloat {
    return random() * (max - min) + min
      ...
      func updateHealthBar(_ node: SKSpriteNode, withHealthPoints hp: Int) {
          node.zPosition = 14
        node.position = CGPoint(x: frame.width*0.18 - 21, y: frame.height*0.9)
        // aumento lunghezza ed altezza
        let barSize = CGSize(width: healthBarWidth*2.27, height: healthBarHeight*0.72);
...
//    anima l'attacco player, dopo tot sec aggiunge il nodo proiettile e lo anima
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.25){
            ...
            let audioAction = SKAction.playSoundFileNamed("Sparo_Personaggio", waitForCompletion: false)
...
/*    COSA SUCCEDE QUANDO ABBIOAMO UNA COLLISIONE:
  ...
  func projectileDidCollideWithbat(projectile: SKSpriteNode, bat: SKSpriteNode) {
    //modifica pedro
    ...
    func batDidColliedWithShield(bat: SKSpriteNode, shield: Shield){
        print("bat hit Player")
        bat.removeFromParent()
    }
  ...
  func didBegin(_ contact: SKPhysicsContact) {
    ...
    func collisionMob(contact: SKPhysicsContact) {
      ...
      if firstBody != nil && secondBody != nil{
          if ((firstBody!.categoryBitMask & PhysicsCategory.bat != 0) &&
              (secondBody!.categoryBitMask & PhysicsCategory.projectile != 0)) {
              if let bat = firstBody!.node as? SKSpriteNode,
              let projectile = secondBody!.node as? SKSpriteNode {
                  projectileDidCollideWithbat(projectile: projectile, bat: bat)
              }
             ...
             if firstBody != nil && secondBody != nil{
                 if ((firstBody!.categoryBitMask & PhysicsCategory.bat != 0 || firstBody!.categoryBitMask & PhysicsCategory.skeleton != 0) &&
                     ...
                     let player = secondBody!.node as? SKSpriteNode {
                        enemyDidColliedWithPlayer(enemy: enemy, player: player as! Player)
                     }
